    ...
    while second_parser.has_more_commands():
        ...
        match second_parser.command_type():
            case Command.C_COMMAND:
                # print(second_parser.dest())
                # print(code.dest(second_parser.dest()), code.comp(second_parser.comp()), code.jump(second_parser.jump()))
                output_list.append("111" + code.comp(second_parser.comp()) + code.dest(second_parser.dest()) + code.jump(second_parser.jump()))
            case Command.A_COMMAND:
                if not symbol_table.contains(second_parser.symbol()):
                    if str.isdigit(second_parser.symbol()):
                        ...
                        a_address += 1
                output_list.append(format(int(symbol_table.getAddress(second_parser.symbol())), "016b"))
                # print(second_parser.symbol())
